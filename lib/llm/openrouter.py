...
class OpenRouterLLM(BaseLLM):
    ...
    def _call_openrouter(self, messages: list, schema: dict = None, max_tokens: int = 100000) -> str:
    ...
    def make_json(self, prompt: str, schema: dict = None, max_tokens: int = 100000) -> dict:
        messages = []
    ...
    def make_image(
        ...
        def _call():
            ...
            try:
                ...
                logger.error("Unexpected image response: %s", data)
    ...
    def edit_image(self, src_img, prompt: str, refs=None,
        ...
        def _call():
            ...
            payload = {
                ...
                "modalities": ["image", "text"],
                "messages": [{"role": "user", "content": contents}],
                "config": {"temperature": 0.25, "seed": int(os.getenv('AI_SEED', '21')), "top_p": 0.8},
            }
            data = self._post_openrouter(payload, timeout=180)
            try:
                image_url = data["choices"][0]["message"]["images"][0]["image_url"]["url"]
