...
from lib.studio.screenwriter import (
    analyze_scenes_master, apply_spatial_disposition_pass, apply_spatial_rewrite_pass,
    merge_scenes, process_single_scene, run_scenes_pipeline, _write_episode_checkpoint,
    _swap_view_ref,
)
...
def cmd_reverse_refine(args):
    """Reversal pass on an already-generated raw episode JSON, then upsert into metadata."""
    try:
...
def cmd_disposition(args):
    ...
    for scene in scenes:
        ...
        # All non-canonical view suffixes that map back to their anchor-holding canonical ref.
        ...
        def _resolve_ref(panel):
            for ref in panel.get('location_references', []):
                ...
                if ref.endswith(('-View-From-Entrance', '-View-Primary', '-Interior-From-Entrance', '-Exterior')) \
                        and ref in anchors_by_ref:
                    return ref
                # Derived view → strip suffix and look up canonical
                for derived_suffix, canonical_suffix in _TO_CANONICAL_SUFFIX.items():
                    if ref.endswith(derived_suffix):
                        canonical = ref[: -len(derived_suffix)] + canonical_suffix
...
def cmd_spatial_rewrite(args):
    """Spatial rewrite pass: fix visual_start / motion_prompt contradictions using visual_disposition."""
    scene_arg = args.scene
    scene_filter = None
    if scene_arg != 'all':
        try:
    ...
    meta_path = project.output_dir / "animation_metadata.json"
    if not meta_path.exists():
        logger.error("❌ animation_metadata.json not found. Run 'make disposition' first.")
        sys.exit(1)
    try:
        metadata = json.loads(meta_path.read_text(encoding='utf-8'))
    except json.JSONDecodeError as e:
    ...
    for scene in scenes:
        ...
        # Group consecutive panels by anchor ref (same grouping logic as disposition).
        groups: list[tuple[str | None, list[dict]]] = []
        for panel in panels:
            ref = _resolve_ref(panel)
            if groups and groups[-1][0] == ref:
        ...
        for anchor_ref, group_panels in groups:
            ...
            if not anchor_points:
                ...
                continue
            # Check that at least one panel in group has visual_disposition set.
            if not any(p.get('visual_disposition') for p in group_panels):
                logger.info(
                    f"  ⏭  Scene {sid} anchor={anchor_ref}: no visual_disposition — run 'make disposition' first"
                )
                continue

            logger.info(
                f"  ✏️  Scene {sid} [{scene.get('location', '')}] "
                f"anchor={anchor_ref} panels={[p.get('panel_index') for p in group_panels]} ..."
            )
            sub_scene = {**scene, 'panels': group_panels}
            apply_spatial_rewrite_pass(sub_scene, anchor_points, llm)
            updated = {p.get('panel_index'): p for p in sub_scene['panels']}
            for i, p in enumerate(scene['panels']):
                if p.get('panel_index') in updated:
                    scene['panels'][i] = updated[p.get('panel_index')]
    ...
    if not processed:
        logger.warning("  ⚠️  No scenes processed — check that disposition pass has been run.")
        return

    from lib.core.utils import atomic_write
    atomic_write(meta_path, json.dumps(metadata, ensure_ascii=False, indent=2))
    logger.info(f"✅ animation_metadata.json updated: {processed} scene(s) with spatial rewrites")
...
def register(sub):
    ...
    p = sub.add_parser('reverse-refine', help='Reversal pass on existing raw episode JSON')
    ...
    p = sub.add_parser(
        'spatial-rewrite',
        help='Fix visual_start/motion_prompt spatial contradictions using visual_disposition (run after disposition)',
    )
    p.add_argument('scene', nargs='?', default='all', help='Scene number or "all" (default: all)')
    p.set_defaults(func=cmd_spatial_rewrite)
