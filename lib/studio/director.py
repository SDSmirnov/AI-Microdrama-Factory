...
def enrich_and_regenerate_reference(
    ...
    is_variation = bool(ref_data.get('character_ref'))  # variation refs have a parent character

    if is_variation:
        # Variation refs represent intentional costume/context-specific looks.
        # All usages are by design (outfit is only worn in specific scenes), so temporal
        # filtering would strip valid outfit details. Treat like a location: add ALL details.
        temporal_rules = """
    TEMPORAL RULES (character variation — outfit/context specific):
    This reference is a VARIATION of a base character (specific outfit or context).
    Add ALL visually consistent details observed across scenes — the outfit is intentional.
    DO NOT add transient states (injury, dirt, wetness) — only permanent outfit/accessory details.
    DO NOT add behavioral signatures or physical vocabulary — those are inherited from the base character ref.
"""
    elif is_character:
        temporal_rules = """
...
"""
    ...
    existing_behavioral = ""
    if is_character and not is_variation:
        bsig = ref_data.get('behavioral_signature', '')
        pvoc = ref_data.get('physical_vocabulary', '')
        if bsig or pvoc:
            existing_behavioral = f"""
    <EXISTING_BEHAVIORAL_DNA>
    behavioral_signature: {bsig}
    physical_vocabulary: {pvoc}
    </EXISTING_BEHAVIORAL_DNA>
    Merge new behavioral evidence from scenes into these — do NOT discard existing entries.
    Characters only: return updated behavioral_signature and physical_vocabulary fields.
"""
        else:
            existing_behavioral = "No existing behavioral DNA — derive from scene usages if sufficient evidence exists. Characters only."
    ...
    For characters: also return updated behavioral_signature and physical_vocabulary merging existing + new evidence.
    For non-characters: return empty strings for behavioral_signature and physical_vocabulary.
    ...
    if is_character and not is_variation:
        if updated_desc.get('behavioral_signature'):
            ref_data['behavioral_signature'] = updated_desc['behavioral_signature']
        if updated_desc.get('physical_vocabulary'):
            ref_data['physical_vocabulary'] = updated_desc['physical_vocabulary']
...
def align_scene_prompts(scene: Dict, all_refs_data: Dict, llm: BaseLLM) -> Dict:
    ...
    for ref in scene_refs:
        ...
        if ref_key in all_refs_data:
            ...
            bsig = all_refs_data[ref_key].get('behavioral_signature', '')
            pvoc = all_refs_data[ref_key].get('physical_vocabulary', '')
            if bsig or pvoc:
                behavior_context[ref] = {'behavioral_signature': bsig, 'physical_vocabulary': pvoc}
...
"""
    ...
    if behavior_context:
        behavior_block = f"""
    BEHAVIORAL SIGNATURES — use to verify motion_intent and blocking plausibility:
    {json.dumps(behavior_context, indent=2)}
    These are permanent character traits — correct any panel where a character acts against their established behavioral signature without a narrative reason.
"""
