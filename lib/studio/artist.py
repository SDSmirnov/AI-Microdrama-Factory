...
def _generate_room_anchors(ref: dict, llm: BaseLLM) -> dict:
    ...
    prompt = (
        ...
        f"   a) visual_disposition_hint — View-From-Entrance (camera at entrance looking in). "
        f"      Depth increases with Y. State the full camera→scene depth chain: what is nearest camera, "
        f"      what furniture is between camera and character (occlusion), what is in background. "
        f"      Include explicit framing: 'DEPTH: [near] → [mid] → [far background]'. "
        f"      Example: 'host seated at far side of desk (East chair); desk surface in foreground lower-third "
        f"      between camera and host; host visible from mid-chest up above desk edge; South wall behind. "
        f"      DEPTH: entrance door (near camera) → desk foreground → host mid-ground → South wall background'.\n"
        f"   b) visual_disposition_hint_to_entrance — View-To-Entrance (camera at far end looking toward entrance). "
        f"      Depth stack is REVERSED: objects near the far wall are now foreground, entrance is background. "
        f"      State explicitly what fills the foreground frame and what is occluded behind it. "
        f"      Example: 'desk surface fills lower-third foreground (camera past desk looking toward entrance); "
        f"      visitor seated on entrance-side of desk visible from mid-chest up above desk edge; "
        f"      entrance door centered in background. DEPTH: desk foreground → visitor mid-ground → entrance background'.\n"
