...
ПРАВИЛА РЕДАКТУРЫ:
- Исправляй только логические ошибки. Не трогать стиль, метафоры, ритм, авторский голос.
- Не добавлять новые сцены, персонажей, сюжетные повороты.
- Обсценная лексика и сцены насилия допустимы.
- Выдать ТОЛЬКО исправленный текст. Никаких пояснений, комментариев, заголовков от себя."""
    ...
    try:
        fixed = llm.make_text(
            prompt=(
                ...
                f"3) Выдай ТОЛЬКО исправленный полный текст главы {chapter_num} "
                f"без сносок и комментариев.\n\n"
