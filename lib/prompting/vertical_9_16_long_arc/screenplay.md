...
- MOBILE MOTION LAW: The viewer's brain asks one question in the first 2 seconds: "is something happening in this world?" Camera movement (zoom, tilt, push-in) does NOT answer that question — the brain filters it out and evaluates only diegetic action (characters moving, objects changing state, interaction occurring). If no diegetic change is visible within 2 seconds, retention drops — regardless of camera work. HARD CONSTRAINTS:
  - First diegetic change (character body movement, object state change, physical interaction) must occur within 2 seconds. Not a zoom. Not a slow pan. A character or object in the scene changing its physical state.
  - A character sitting and thinking (even dramatically, even with expressive face) = "nothing is happening" = swipe.
  - A figure appearing in frame, even with camera zoom = "figure appeared but nothing happened" = retention drop.
  - A face holding emotion with no body movement = small but measurable retention drop per second held.
  - Every 2 seconds throughout the clip: at least one new diegetic physical event (full-body/large-limb — not eye shift, not expression change).
- FORBIDDEN VISUALS — HARD FAILURE if present anywhere in visual_start, visual_end, or motion_prompt:
  - **Tears** (any form: "tears run down", "eyes fill with tears", "cheeks wet", "crying", "weeping"). Show emotional state through jaw tension, bitten lip, chin tremor, bright/wide eyes, body curl — never visible liquid on the face. Viewers swipe on tears regardless of narrative context.
  - **Sweat** (any form: "forehead glistens", "sweat drips", "damp skin", "perspiration"). Show exertion through posture, breath, movement pace — never liquid on skin.
  - **Spitting** (any form). No exceptions.
  These are viewer-behavior triggers confirmed by retention analytics — they cause immediate scroll-past independent of story quality.
