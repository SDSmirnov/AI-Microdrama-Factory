...
- VOICEOVER IS THE SPINE: Every panel has either dialogue or voiceover. Inner monologue reveals what the image cannot show. Never narrates the obvious. INNER MONOLOGUE HARD LIMIT: 4–5 words only — it is a flash of hidden thought, not a sentence. "He knows." / "Not now." / "This is wrong." People think fast; the viewer reads it in one blink. Any inner monologue voiceover exceeding 5 words is a hard failure — cut to the bone.
- DIALOGUE IS PERFORMANCE: ≤8 words per line. Staccato. Emotionally specific. Delivered in close-up on the speaker's face.
- VOICE BUDGET: 24 chars/sec × panel duration = hard character limit shared by dialogue + voiceover combined. 6s panel = 144 chars total. 4s panel = 96 chars total. Exceed it and TTS truncates or produces garbled audio.
...
- HOOK ARCHITECTURE: Panel 1 of every episode = cold_open (IN MEDIAS RES — drop into mid-action or mid-confrontation, something is already happening, skip all setup). The first frame must contain a visible power dynamic or a stake object already in play. FORBIDDEN: passive character, contemplative expression, establishing shot, beauty without conflict. Emotional peak before midpoint. Final panel of the LAST episode in the series = cliffhanger (response pressure — protagonist at the moment before their response). Final panel of INTERMEDIATE episodes = tension_peak (maximum escalation without resolution, voiceover mandatory).
- COLD OPEN SELF-AUDIT — HARD CHECK: If motion_prompt[0s] contains any of these as the primary state: "stands motionless", "sits still", "is perfectly still", "waits", "holds position", "gazes", "stares", "looks at" — HARD FAILURE. Rewrite: the first word after "At 0s" must be a verb of physical motion or active exchange. The action must be 50%+ complete at frame 0.
- COLD OPEN FORBIDDEN: power display through inaction — a character demonstrating status by NOT reacting while another person waits, speaks, or performs. Sitting still while someone prattles. Staring past someone dismissively. Refusing to acknowledge. Power through ABSENCE of action is passive setup dressed as drama. Power in P1 MUST be shown through an action that provokes a visible, physical reaction from another character in the same frame.
...
Instructions: You must adhere to the following iterative quality control process for every response, before returning response, do step-by-step.
