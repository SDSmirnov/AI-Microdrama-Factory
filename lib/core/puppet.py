...
Coordinate system (per ANCHOR_SCHEMA) — all positions are normalized [0, 1] fractions:
  X = fraction of room width.
    X=0 → image-left wall as seen in the View-From-Entrance reference image.
    X=1 → image-right wall.
    X=0.5 → center.
  Y = fraction of room depth.
    Y=0 → entrance wall.
    Y=1 → far wall (opposite the entrance).
  Z = fraction of room height. Z=0 = floor, Z=1 = ceiling.
  View-To-Entrance: x_mirrored = 1 - x  (image-left↔image-right swap); Y depth reversed.

No compass directions (N/S/E/W) anywhere — coordinates are image-relative only.
room_m = [width_m, depth_m] in anchor_points is kept for physical distance calculations.
...
class SceneState:
    ...
    room_m: list[float] = field(default_factory=lambda: [8.0, 8.0])

    def validate_transitions(self) -> list[dict]:
        """Check consecutive frames for physically impossible position jumps.

        Positions are normalized [0,1] fractions; room_m converts to meters.
        Budget = panel duration * 1.2 m/s walking speed.
        Returns: [{panel_a, panel_b, character, distance_m, budget_m}]
        """
        w = self.room_m[0] if self.room_m else 8.0
        d = self.room_m[1] if len(self.room_m) > 1 else 8.0
        violations = []
        for a, b in zip(self.frames, self.frames[1:]):
            ...
            for name in set(a.characters) & set(b.characters):
                ...
                if not pa.visible or not pb.visible:
                    continue
                dist_m = math.hypot((pb.x - pa.x) * w, (pb.y - pa.y) * d)
                if dist_m > budget_m:
                    violations.append({
...
def camera_from_view_type(view_type: str, anchor_points: dict) -> CameraRig:
    """Construct a CameraRig in normalized [0,1] space, offset just outside room boundary.

    Coordinates are [0,1] fractions: X=0 image-left wall, X=1 image-right wall,
    Y=0 entrance wall, Y=1 far wall. Camera is centered on X (0.5) for unambiguous
    screen-side derivation when no lateral offset is specified.
    """
    if view_type == 'To-Entrance':
        return CameraRig(
            x=0.5, y=1.01, z=0.2,
            look_at_x=0.5, look_at_y=0.0, look_at_z=0.15,
        )
    return CameraRig(
        x=0.5, y=-0.01, z=0.2,
        look_at_x=0.5, look_at_y=1.0, look_at_z=0.15,
    )
