        ...
        "episodes": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "screenplay_instructions": {"type": "string", "description": "Per-panel production blueprint (P1…P9). For each panel: POWER (who controls the space/prop), EMOTION (face physics — micro-expression, not a label), STAKE OBJECT (visible prop carrying the subtext), STATE (dramatic delta from start to end — meaning, not just action), DIALOGUE SEED (≤8 words). When a panel's STATE describes a physical action that begins but does not resolve within the panel — a reach not completed, an approach not arrived, a blow not yet landed — append: THREAD→P[N+1]: [one sentence stating what the next panel's visual_start must open on to resolve this thread]. This is a hard contract between consecutive panels: the scene generator must open the next panel at the resolved or interrupted state, not at an unrelated starting position."},
                },
