...
_MIN_WORD_GAP_SEC = 0.33  # split at inter-word pauses longer than this
...
def _split_on_sentences(segments: list[dict], min_word_gap_sec: float = _MIN_WORD_GAP_SEC) -> list[dict]:
    ...
    some Whisper models). Fallback 1: original_text sentence boundaries mapped
    back to word tokens by word count. Fallback 2: inter-word pause >
    min_word_gap_sec (default 0.33s). Last resort: duration cap — any phrase
    longer than _MAX_PHRASE_SEC is split at word boundaries into equal-duration
    chunks.
    """
    result = []
    for seg in segments:
        ...
        # ── Fallback 2: inter-word pause — natural breath/sentence boundary ──
        gap_splits = [
            i for i in range(len(words) - 1)
            if words[i + 1]["start"] - words[i]["end"] > min_word_gap_sec
        ]
        if gap_splits:
            boundaries = [0] + [i + 1 for i in gap_splits] + [len(words)]
            _emit_chunks(boundaries)
            continue

        # ── Fallback 3: duration cap — split at word boundaries ──
